"""
SiliconFlow Service
封装硅基流动 API（通过 OpenAI SDK），提供 DeepSeek V3.2 支持。
"""
...
class SiliconFlowService:
    """
    封装硅基流动 API，提供与 GeminiService/DeepSeekService 相同的接口。
    使用 OpenAI SDK 兼容调用。
    """
    ...
    def __init__(self, model_name: Optional[str] = None):
        ...
        Args:
            model_name: 模型名称（可选，默认使用 settings.SILICONFLOW_MODEL_NAME）
        """
        if not settings.SILICONFLOW_API_KEY:
            raise ValueError("SILICONFLOW_API_KEY 未配置，请在 .env 中设置")

        self.model_name = model_name or settings.SILICONFLOW_MODEL_NAME
        self._client = OpenAI(
            api_key=settings.SILICONFLOW_API_KEY,
            base_url=settings.SILICONFLOW_BASE_URL,
        )

        logger.info(f"[SiliconFlow] 使用模型: {self.model_name}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if result_text:
                    ...
                    logger.info(f"[SiliconFlow] 响应生成完毕 (模型: {self.model_name})")
                    return result_text
                else:
                ...
                last_error = e
                logger.warning(
                    f"[SiliconFlow] 请求失败 (尝试 {attempt + 1}/{max_retries}): {e}"
                )
                if attempt < max_retries - 1:
                    time.sleep(1)
                    continue

        logger.error(f"[SiliconFlow] 所有重试均失败: {last_error}")
        raise RuntimeError(f"SiliconFlow API 调用失败: {last_error}")
    ...
    def chat(self, full_prompt: str) -> str:
        ...
        logger.info("[SiliconFlow] 开始聊天请求...")
    ...
    def analyze_training(self, daily_report_md: str) -> str:
        ...
        if not daily_report_md or not daily_report_md.strip():
            logger.warning("[SiliconFlow] 收到空数据，无法进行分析")
            return "## 📊 分析结果\n\n暂无数据，无法进行分析。"

        logger.info("[SiliconFlow] 开始请求训练分析...")
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(f"[SiliconFlow] 训练分析失败: {error_msg}")
            return f"""## 📊 分析结果
        ...
        *,
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise ValueError("SiliconFlow 返回非 JSON 内容")
            ...
            logger.warning(f"[SiliconFlow] Home summary brief failed: {e}")
            return {"week": None, "month": None}
