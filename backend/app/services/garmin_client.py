...
class GarminClient:
    ...
    def get_health_stats(self, date_str: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if user_summary:
                ...
                # HRV Status
                hrv_status = user_summary.get("hrvStatus") or user_summary.get(
                    "hrvStatusDTO"
                )
                if hrv_status:
                    if isinstance(hrv_status, dict):
                        result["hrv_status"] = hrv_status.get(
                            "status"
                        ) or hrv_status.get("value")
                    else:
                        result["hrv_status"] = str(hrv_status)

                # Body Battery: chargedValue 和 drainedValue
                if "bodyBatteryMostRecentValue" in user_summary:
                    result["body_battery"] = user_summary.get(
                        "bodyBatteryMostRecentValue"
                    )

                # 尝试从 bodyBatteryDTO 或其他字段获取 charged/drained
                bb_dto = user_summary.get("bodyBatteryDTO") or user_summary.get(
                    "bodyBattery"
                )
        ...
        try:
            ...
            if sleep_data:
                ...
                if "sleepTimeSeconds" in daily_sleep:
                    ...
                    if sleep_time_sec is not None and isinstance(
                        sleep_time_sec, (int, float)
                    ):
                        result["sleep_time_hours"] = round(
                            float(sleep_time_sec) / 3600, 1
                        )
                ...
                if deep_sleep_sec is not None and isinstance(
                    deep_sleep_sec, (int, float)
                ):
                ...
                if rem_sleep_sec is not None and isinstance(
                    rem_sleep_sec, (int, float)
                ):
                ...
                if light_sleep_sec is not None and isinstance(
                    light_sleep_sec, (int, float)
                ):
                ...
                if awake_sleep_sec is not None and isinstance(
                    awake_sleep_sec, (int, float)
                ):
                ...
                # 获取睡眠结束时间（用于 Body Battery）
                sleep_end_ts = daily_sleep.get(
                    "sleepEndTimestampGMT"
                ) or daily_sleep.get("sleepEndTimestampLocal")
                result["sleep_end_timestamp"] = sleep_end_ts
    ...
    def get_training_plan(
        self, start_date_str: str, days: int = 3
    ) -> List[Dict[str, Any]]:
        """
        获取未来几天的训练计划。
                ...
                try:
                    ...
                    while current_date <= end_date:
                        ...
                        try:
                            ...
                            day_data = self.client.connectapi(
                                calendar_url, params=params
                            )
            ...
            try:
                ...
                for goal in goals or []:
                    if isinstance(goal, dict):
                        goal_date = (
                            goal.get("targetDate")
                            or goal.get("startDate")
                            or goal.get("date")
                        )
                        if goal_date:
                            # 检查日期是否在范围内
                            try:
                                goal_date_obj = datetime.strptime(
                                    str(goal_date)[:10], "%Y-%m-%d"
                                )
    ...
    def get_mock_data(
        ...
        Optional[Dict[str, Any]], Optional[Dict[str, Any]], List[Dict[str, Any]]
    ]:
        """
        ...
        mock_file = os.path.join(
            current_dir, "..", "..", "..", "backend", "app", "data", "mock_garmin.json"
        )
        ...
        if not target_day:
            ...
            monthly_files = glob.glob(
                os.path.join(project_root, "garmin_monthly_*.json")
            )
                ...
                try:
                    ...
                    for day in days:
                        if day.get("date") == target_date:
                            ...
                            logger.info(
                                f"[Garmin] 在 {os.path.basename(monthly_file)} 中找到日期 {target_date} 的数据"
                            )
                            break
        ...
        if sleep_info:
            ...
            health["sleep_time_hours"] = (
                round(sleep_info.get("total_duration", 0) / 3600, 1)
                if sleep_info.get("total_duration")
                else None
            )
            health["sleep_score"] = sleep_info.get("sleep_score")
            health["deep_sleep_seconds"] = sleep_info.get("deep_sleep_seconds")
            health["deep_sleep_hh_mm"] = _seconds_to_hh_mm(
                sleep_info.get("deep_sleep_seconds")
            )
            health["rem_sleep_seconds"] = sleep_info.get("rem_sleep_seconds")
            health["rem_sleep_hh_mm"] = _seconds_to_hh_mm(
                sleep_info.get("rem_sleep_seconds")
            )
            health["light_sleep_seconds"] = sleep_info.get("light_sleep_seconds")
            health["light_sleep_hh_mm"] = _seconds_to_hh_mm(
                sleep_info.get("light_sleep_seconds")
            )
            health["awake_sleep_seconds"] = sleep_info.get("awake_sleep_seconds")
            health["awake_sleep_hh_mm"] = _seconds_to_hh_mm(
                sleep_info.get("awake_sleep_seconds")
            )
            health["recovery_quality_percent"] = sleep_info.get(
                "recovery_quality_percent"
            )
        ...
        health["average_stress_level"] = summary.get(
            "average_stress_level", 35
        )  # 默认值 35
        health["stress_qualifier"] = summary.get(
